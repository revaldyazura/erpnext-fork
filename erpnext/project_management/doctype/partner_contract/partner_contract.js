...
frappe.ui.form.on("Partner Contract", {
	refresh(frm) {
		if (!frm.is_new()) {
			...
			frm.add_custom_button("Show Documents", function () {
				...
				const make_doc_dialog = () => {
					...
					(function ensure_full_doc_style() {
						const fullCSS = `
						...
						.wide-documents-dialog .form-layout, .wide-documents-dialog .form-page, .wide-documents-dialog .form-section, .wide-documents-dialog .section-body { width:100% !important; max-width:100% !important; margin:0; padding:0; }
						.wide-documents-dialog .form-column { width:100% !important; max-width:100% !important; flex:0 0 100%; padding:0; }
						.wide-documents-dialog .frappe-control[data-fieldname="docs_html"], .wide-documents-dialog .frappe-control[data-fieldname="pagination_html"] { width:100% !important; margin:0; padding:0; }
					...
					`;
						let styleEl = document.getElementById("doc-dialog-style-fixed");
						if (!styleEl) {
							styleEl = document.createElement("style");
							styleEl.id = "doc-dialog-style-fixed";
							styleEl.textContent = fullCSS;
							document.head.appendChild(styleEl);
						} else if (!/\.wide-documents-dialog \.form-layout/.test(styleEl.textContent)) {
							styleEl.textContent = fullCSS; // upgrade
						}
					})();
					setTimeout(() => {
						dlg.$wrapper
								...
								frappe.db
									...
									.then((r) => {
										...
										const doc = {
											...
											related_document_name: frm.doc.partner_name,
