...
frappe.ui.form.on("Requirements", {
	after_save(frm) {
		if (!frm.is_new()) {
			frappe.call({
				method: "erpnext.project_management.doctype.requirements.requirements.maybe_rename", // buat method ini
	...
	refresh(frm) {
		if (frm.is_new()) {
			frm.set_df_property("requirement_status", "options", ["", "Pending"]);
			frm.set_value("requirement_status", "");
			// Hide validation related fields while creating a new Requirements doc
			[
				"tech_validation",
				"validation_date",
				"validation_by",
			...
			// Always show the tech_validation checkbox in edit mode
			frm.toggle_display("tech_validation", true);
			// Show other validation fields only if tech_validation is checked
			const show_details = !!frm.doc.tech_validation;
			["validation_date", "validation_by", "validation_reason", "validation_by_name"].forEach(
				(field) => {
					frm.toggle_display(field, show_details);
				}
			);
			// If already checked but missing autofill fields, populate them
			if (show_details) {
				maybe_autofill_validation_fields(frm);
			}
			frappe.db.get_value("Pipeline Projects", frm.doc.pipeline_id, "pipeline_status").then((r) => {
				if (r && r.message) {
					if (r.message.pipeline_status == "Converted" && frappe.session.user !== "Administrator") {
			...
			frm.add_custom_button(
				...
				function () {
					...
					const make_dialog = () => {
						...
						dlg.$wrapper.addClass("wide-changelogs-dialog");
						if (!document.getElementById("req-dialog-style-fixed")) {
							const style = document.createElement("style");
							style.id = "req-dialog-style-fixed";
							style.textContent = `
                        .wide-changelogs-dialog .modal-dialog { max-width:85vw; width:85vw; }
                        .wide-changelogs-dialog .modal-content { width:100%; }
                        .wide-changelogs-dialog .modal-body { max-height:72vh; overflow:auto; padding: 8px 14px 12px; }
                        .wide-changelogs-dialog .form-layout, 
                        .wide-changelogs-dialog .form-page, 
                        .wide-changelogs-dialog .form-section, 
                        .wide-changelogs-dialog .section-body { width:100% !important; max-width:100% !important; margin:0; padding:0; }
                        .wide-changelogs-dialog .form-column { width:100% !important; max-width:100% !important; flex:0 0 100%; padding:0; }
                        .wide-changelogs-dialog .frappe-control { margin-bottom:6px; }
                        .wide-changelogs-dialog .frappe-control[data-fieldname="changelogs_html"],
                        .wide-changelogs-dialog .frappe-control[data-fieldname="pagination_html"],
                        .wide-changelogs-dialog .frappe-control[data-fieldname="bottom_actions"] { width:100% !important; margin:0; padding:0; }
                        .wide-changelogs-dialog .req-table-wrapper { width:100%; }
                        .wide-changelogs-dialog .req-table { width:100%; table-layout:auto; }
                        .wide-changelogs-dialog .req-table th { white-space:nowrap; }
                        .wide-changelogs-dialog .req-table td { white-space:nowrap; }
                        .wide-changelogs-dialog .req-table td.id-cell { white-space:normal; word-break:normal; font-family:inherit; }
                        .wide-changelogs-dialog .req-table td.desc-cell { white-space:normal; line-height:1.3; max-width:460px; width:35%; overflow:hidden; }
                        .wide-changelogs-dialog .req-table td.action-cell { white-space:normal; }
                        .wide-changelogs-dialog .req-table td.action-cell .btn { display:block; width:100%; margin:0 0 4px 0; }
                        .wide-changelogs-dialog .req-table td.action-cell .btn:last-child { margin-bottom:0; }
                        .wide-changelogs-dialog .req-table td.text-center { text-align:center; }
                        .wide-changelogs-dialog .tech-val-icon { display:inline-block; width:18px; font-weight:600; color: var(--green, #2e7d32); }
                        .wide-changelogs-dialog .tech-val-icon.off { color:#bbb; }
                        @media (max-width: 1200px) {
                            .wide-changelogs-dialog .req-table th, .wide-changelogs-dialog .req-table td { white-space:normal; }
                        }
                    `;
						...
						const columns = [
							{ key: "name", label: "ID" },
							{ key: "requirements_id", label: "Requirements ID" },
							{ key: "change_by", label: "Change By" },
							{ key: "change_by_name", label: "Change By Name" },
						...
						function fetch_and_render() {
							frappe.call({
								method: "erpnext.project_management.doctype.requirements.requirements.get_changelogs",
								args: {
									requirements_id: frm.doc.name,
									page: state.page,
						...
						function render_table(rows, total) {
							...
							if (!rows.length) {
								...
								rows.forEach((row) => {
									html +=
										...
										` data-requirements_id="${esc(
											row.requirements_id || ""
										)}" data-date="${esc(row.date || "")}"` +
										` data-change_by="${esc(
											row.change_by || ""
										)}" data-change_by_name="${esc(row.change_by_name || "")}"` +
									...
									columns.forEach((c) => {
										...
										} else if (c.key === "change_description") {
											const raw = row.change_description || "";
							...
							dlg.fields_dict.changelogs_html.$wrapper.html(html);
							bind_row_events();
						}
						...
						function bind_row_events() {
							...
							dlg.$wrapper
								...
								.on("click", function () {
									...
									frappe.set_route("Form", "Changelogs", docname);
								});
						}
									...
									frappe.db
										...
										.then((r) => {
											...
											const doc = {
												...
												related_document_name: frm.doc.title,
		...
		toggle_origin_fields(frm);
	},
	onload: function (frm) {
		// Hanya tampilkan Pipeline Projects yang belum Converted
		frm.set_query("pipeline_id", function () {
			return {
				filters: [["Pipeline Projects", "pipeline_status", "!=", "Converted"]],
			};
		});
	},
	pipeline_id(frm) {
		toggle_origin_fields(frm);
		if (frm.doc.pipeline_id && frm.doc.project_id) {
			// Jika user memilih pipeline setelah project terisi, kosongkan project agar konsisten single origin
			frm.set_value("project_id", null);
		}
	},
	project_id(frm) {
		toggle_origin_fields(frm);
		if (frm.doc.project_id && frm.doc.pipeline_id) {
			// Jika user memilih project setelah pipeline terisi, kosongkan pipeline agar konsisten single origin
			frm.set_value("pipeline_id", null);
		}
	},
	tech_validation(frm) {
		if (frm.is_new()) return; // new doc: fields sudah disembunyikan
		const show_details = !!frm.doc.tech_validation;
		["validation_date", "validation_by", "validation_reason", "validation_by_name"].forEach((field) => {
			frm.toggle_display(field, show_details);
		});
		if (show_details) {
			maybe_autofill_validation_fields(frm, { force: false });
		} else {
			...
			frm.set_value({
				...
				validation_by_name: null,
			});
		}
	},
});

// Helper: autofill validation_date (today) & validation_by (current user's Employee) if empty
function maybe_autofill_validation_fields(frm, opts) {
	opts = opts || {};
	if (!frm || !frm.doc) return;
	// Only proceed if tech_validation is truthy
	if (!frm.doc.tech_validation) return;

	const needs_date = !frm.doc.validation_date || opts.force;
	const needs_by = !frm.doc.validation_by || opts.force;
	const needs_by_name = !frm.doc.validation_by_name || opts.force;
	if (!needs_date && !needs_by && !needs_by_name) return;

	// Set today's date (client side) in user default format; Frappe will normalize
	if (needs_date) {
		frm.set_value("validation_date", frappe.datetime.get_today());
	}
	...
	if (needs_by || needs_by_name) {
		...
		frappe.db
			...
			.then((r) => {
				...
				if (needs_by && emp) {
					frm.set_value("validation_by", emp);
				}
				if (needs_by_name && emp_name) {
					frm.set_value("validation_by_name", emp_name);
				}
			});
	}
}

function toggle_origin_fields(frm) {
	if (!frm || !frm.doc) return;
	const has_pipeline = !!frm.doc.pipeline_id;
	const has_project = !!frm.doc.project_id;

	if (!has_pipeline && !has_project) {
		// Keduanya kosong: tampilkan dua-duanya
		frm.toggle_display("pipeline_id", true);
		frm.toggle_display("project_id", true);
	} else if (has_pipeline) {
		frm.toggle_display("pipeline_id", true);
		frm.toggle_display("project_id", false);
	} else if (has_project) {
		frm.toggle_display("pipeline_id", false);
		frm.toggle_display("project_id", true);
	}
}
