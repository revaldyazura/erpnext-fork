...
frappe.ui.form.on("Client", {
								...
								frappe.db
									...
									.then((r) => {
										...
										const doc = {
											...
											related_document_name: frm.doc.client_name,
	...
	onload(frm) {
		const target_fields = ["client_alias", "sub_unit_alias"]; // daftar field yang dipaksa uppercase

		const attach_uppercase = (fieldname) => {
			const field = frm.fields_dict[fieldname];
			if (!field) return;

			// Set nilai awal (jika sudah ada) ke uppercase
			const cur_val = frm.doc[fieldname];
			if (cur_val) {
				const up = cur_val.toUpperCase();
				if (up !== cur_val) field.set_value(up);
			}

			const $input = field.$input || field.$wrapper.find("input");
			if (!$input || !$input.length) return;
			if ($input.data("upper-bound")) return; // cegah double binding
			$input.data("upper-bound", true);
			...
			$input.on("input", function () {
				...
				if (current !== upper) {
					this.value = upper;
					frm.doc[fieldname] = upper; // update model langsung (hemat)
					frm.dirty();
				}
			});
		};
		...
		frappe.after_ajax(() => {
			setTimeout(() => {
				target_fields.forEach((f) => attach_uppercase(f));
			}, 300); // beri waktu render
		});
	},
	client_alias(frm) {
		if (frm.doc.client_alias) {
			const up = frm.doc.client_alias.toUpperCase();
			if (up !== frm.doc.client_alias) frm.set_value("client_alias", up);
		}
	},
	sub_unit_alias(frm) {
		if (frm.doc.sub_unit_alias) {
			const up = frm.doc.sub_unit_alias.toUpperCase();
			if (up !== frm.doc.sub_unit_alias) frm.set_value("sub_unit_alias", up);
		}
	},
});
