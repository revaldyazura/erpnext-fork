...
frappe.ui.form.on("Partner Projects", {
	refresh(frm) {
		if (!frm.is_new()) {
			...
			frm.add_custom_button("Show Documents", function () {
				...
				const make_doc_dialog = () => {
					...
					// Ensure upgraded (full-width) style for documents dialog; replace older minimal version if detected
					(function ensure_full_doc_style() {
						const fullCSS = `
							...
							.wide-documents-dialog .section-body { width:100% !important; max-width:100% !important; margin:0; padding:0; }
							...
							.wide-documents-dialog .frappe-control[data-fieldname="pagination_html"] { width:100% !important; margin:0; padding:0; }
						...
						if (!styleEl) {
							...
							// Upgrade existing minimal CSS to full version
							styleEl.textContent = fullCSS;
						}
					})();
					setTimeout(() => {
